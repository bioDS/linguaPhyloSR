 ...
 */
public class SimSRFBDAge implements GenerativeDistribution<TimeTree> {
    ...
    public SimSRFBDAge(@ParameterInfo(name = lambdaParamName, description = "per-lineage birth rate.") Value<Number> birthRate,
...
//        RandomUtils.setSeed(42);
    ...
    @GeneratorInfo(name = "SimSRFBDAge",
            category = GeneratorCategory.BD_TREE, examples = {"simSRFBDAge.lphy"},
            description = "A tree of extant species and those sampled through time (in stratigraphic ranges), which is conceptually embedded in a full species tree produced by a speciation-extinction (birth-death) branching process.<br>" +
